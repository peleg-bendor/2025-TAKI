	...
	public class DontDestroyOnLoad : MonoBehaviour {
		...
		private void Awake () {
			...
			if (existingObjects.Length > 1) {
				...
				TakiLogger.LogInfo ($"Duplicate {uniqueTag} object found. Destroying duplicate.", TakiLogger.LogCategory.System);
				Destroy (gameObject);
				return;
			}
			...
			TakiLogger.LogInfo ($"{gameObject.name} marked as persistent and will not be destroyed on scene load.", TakiLogger.LogCategory.System);
		}
	}
}